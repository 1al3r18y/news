...
export class PanelLayoutManager implements AppModule {
  ...
  renderLayout(): void {
    this.ctx.container.innerHTML = `
      <div class="header">
        ...
        return `
          ...
          <span class="logo">العالم الآن لحظة بلحظة</span>
          ...
          <div class="region-selector">
            ...
            <button id="layersToggleBtn" class="layers-toggle-btn">${t('components.deckgl.layersTitle', 'الطبقات')} ▾</button>
          </div>
        </div>
    ...
    // Layers toggle button handler
    const layersBtn = document.getElementById('layersToggleBtn');
    let layersCloseHandler: ((ev: MouseEvent) => void) | null = null;

    const closeLayersPanel = (panel: HTMLElement) => {
      panel.classList.remove('layers-dropdown-open');
      layersBtn?.classList.remove('active');
      if (layersCloseHandler) {
        document.removeEventListener('click', layersCloseHandler);
        layersCloseHandler = null;
      }
    };
    ...
    layersBtn?.addEventListener('click', (e) => {
      e.stopPropagation();
      e.preventDefault();
      const panel = document.querySelector('.deckgl-layer-toggles') as HTMLElement;
      if (!panel) {
        // Map may not have initialized yet — retry after a short delay
        setTimeout(() => {
          const retryPanel = document.querySelector('.deckgl-layer-toggles') as HTMLElement;
          if (retryPanel) {
            retryPanel.classList.add('layers-dropdown-open');
            layersBtn.classList.add('active');
          }
        }, 500);
        return;
      }
      const isOpen = panel.classList.toggle('layers-dropdown-open');
      layersBtn.classList.toggle('active', isOpen);
      if (isOpen) {
        // Ensure panel is visible by scrolling into view on mobile
        panel.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
        // Close when clicking outside
        layersCloseHandler = (ev: MouseEvent) => {
          const target = ev.target as Node;
          if (!panel.contains(target) && !(layersBtn as Node).contains(target)) {
            closeLayersPanel(panel);
          }
        };
        setTimeout(() => {
          if (layersCloseHandler) document.addEventListener('click', layersCloseHandler);
        }, 10);
      } else {
        // Toggled closed — clean up close handler
        closeLayersPanel(panel);
      }
    });
